(() => {
  ...
  const showAccount = (data) => {
    ...
    if (data.license) {
      ...
      key.closest('.trial-key-row').hidden = true;
      key.previousElementSibling.hidden = true;
  ...
  const loadAccount = async () => {
    try { showAccount(await request('/v1/account')); }
    catch { setStatus('Sign in with Google to create your trial.', 'neutral'); }
  };
    ...
    try {
      ...
      google.accounts.id.initialize({
        ...
        callback: async ({ credential }) => {
          ...
          try {
            const result = await request('/v1/auth/google', { method: 'POST', body: JSON.stringify({ credential }) });
  ...
  create?.addEventListener('click', async () => {
    ...
    try {
      ...
      action.hidden = true;
      license.hidden = false;
      key.closest('.trial-key-row').hidden = false;
      key.previousElementSibling.hidden = false;
      key.value = result.license_key;
      meta.textContent = `One device · expires ${new Date(result.expires_at).toLocaleString()}`;
      setStatus('Trial license created. Copy and save the key now.', 'success');
    } catch (error) { setStatus(error.message, 'error'); }
    finally { create.disabled = false; }
  });
  copy?.addEventListener('click', async () => {
    try { await navigator.clipboard.writeText(key.value); copy.textContent = 'Copied'; setTimeout(() => { copy.textContent = 'Copy'; }, 1600); }
    catch { key.select(); document.execCommand('copy'); }
  });
  logout?.addEventListener('click', async () => {
    try { await request('/v1/auth/logout', { method: 'POST', body: '{}' }); } catch {}
    account.hidden = true; action.hidden = true; license.hidden = true; googleMount.hidden = false;
